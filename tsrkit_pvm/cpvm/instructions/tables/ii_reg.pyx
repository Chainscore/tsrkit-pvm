...
# ---- C helper functions ----#
# ----------------------------#
cdef uint32_t _count_set_bits_c(uint64_t val):
...
cdef uint32_t _leading_zeros_c(uint64_t val, uint32_t bitsize):
...
cdef uint32_t _trailing_zeros_c(uint64_t val, uint32_t bitsize):
