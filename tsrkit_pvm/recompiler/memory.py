...
class GuestMemory:
    ...
    def __init__(self, jump_len: int):
        """
        ...
        self.buf = mmap.mmap(
            -1,
            length=PVM_MEMORY_TOTAL_SIZE + VMContext.calculate_size(jump_len),
            flags=mmap.MAP_ANONYMOUS | mmap.MAP_PRIVATE,
        )
        self.buf_start = ctypes.addressof(ctypes.c_char.from_buffer(self.buf))
        self.offset = self.buf_start + VMContext.calculate_size(jump_len)

    @classmethod
    def from_initial(cls, initial_page_map: list, initial_data: list, jump_len: int):
        ...
        for pm in initial_page_map:
            ...
            start_addr = mem.buf_start + VMContext.calculate_size(jump_len) + pm["address"]
        ...
        for data in initial_data:
            ...
            guest_offset = VMContext.calculate_size(jump_len) + data["address"]
