...
class PVM:
    ...
    def execute(
        ...
        if logger: logger.debug("Starting PVM execution", registers=registers, inst_size=len(program.instruction_set), initial_pc=program_counter, initial_gas=gas, program_size=len(program.zeta))
        ...
        while True:
            try:
                ...
                if remaining_gas < 0:
                    if logger: logger.warning("PVM - OUT_OF_GAS", final_pc=program_counter, gas_deficit=abs(remaining_gas))
                    status = OUT_OF_GAS
                    break

                elif status == ExecutionStatus.HALT:
                    if logger: logger.info("PVM - HALT", final_pc=program_counter, gas_remaining=remaining_gas)
                    break
                elif status == ExecutionStatus.HOST:
                    if logger: logger.debug("PVM - HOST", pc=program_counter, gas_remaining=remaining_gas)
                    break

            except PvmError as e:
                if logger: logger.error("PVM execution error", error_message=str(e), error_code=e.code, pc=program_counter, gas_remaining=remaining_gas)
                if e.code == PANIC:
                    status = PANIC
                    break
                ...
                if logger: logger.critical("Unexpected PVM execution error", error=str(e), error_type=type(e).__name__, pc=program_counter)
                raise e

        if logger: logger.info("PVM result", final_pc=program_counter, gas_remaining=remaining_gas, registers=registers, memory=memory)
